(function () {
    const __Mcop = function (scope) {
        ...
        this.utils = {
            ...
            mcopRealUriHttpVarName: 'Mcop-Real-Uri',
            ...
            originalHostName: 'original-host',
            proxyMethods: {
                PROXY_ALL_FILES: {
                    value: 0,
                    regExp: null
    ...
    __Mcop.prototype.isRelativeUrl = function (url) {
        const _url = (url + '').replace(/\s*/mg, '');
        return /^\//.test(_url) && ! /^\/\//.test(_url);
    };
    ...
    /**
     *
     * @param {string} originalUrl a url to rewrite
     * @returns {string}
     */
    __Mcop.prototype.modifyUrl = function (originalUrl) {
        if (typeof originalUrl !== 'string')
            return originalUrl;


        if (/data:|blob:|javascript:|about:blank|^#|^\*$/.test(originalUrl) || originalUrl.length === 0) {
            return originalUrl;
        }


        const thisProxy = this;
        let newUrl = (originalUrl.includes('#')) ? originalUrl : originalUrl.replace(/\s*/mg, '').replace(/\?/g, '&').replace(/&/, '?');
        const ORIGINAL_HOST = thisProxy.utils.originalHostName;
        ...
        if (! this.isAbsoluteUrl(originalUrl)) {
            if (/^\/\//.test(originalUrl)) {
                newUrl = locationUsed.protocol + originalUrl;
            } else {
                if (/^\//.test(originalUrl)) {
                    newUrl = locationUsed.origin + originalUrl;
                } else {
                    newUrl = locationUsed.origin + '/' + originalUrl;
                }

                /*const realHost = this.getRealHost();
                if (typeof realHost === 'string' && realHost.length > 0 && ! originalUrl.includes('original-host')) {
                    if (thisProxy.urlContainsSearchPart(originalUrl)) {
        ...
        if (originalUrl.startsWith(locationUsed.origin)) {
            return originalUrl;
        }
        ...
        const urlHostname = targetedUrlObjt.host;
        const pageHostname = thisProxy.getRealHost();

        newUrl += targetedUrlObjt.search;

        if (urlHostname !== pageHostname) {
            newUrl += (thisProxy.urlContainsSearchPart(newUrl)) ? '&' + ORIGINAL_HOST + '=' + urlHostname : '?' + ORIGINAL_HOST + '=' + urlHostname;
        }
    ...
    /**
     * Returns true in case url is an absolute url with the original-host get variable; and false otherwise.
     * @param url {string} an absolute url that is checked.
     * @return {boolean}
     */
    __Mcop.prototype.isModifiedUrl = function(url) {
        return (typeof url === 'string') &&
            this.isAbsoluteUrl(url) &&
            (new RegExp(`${this.utils.originalHostName}=.+`)).test(url);
    }

    /**
     * Returns the original version of a url that was modified by the **modifyUrl** function.
     * @param modifiedUrl {string} an absolute url that should be converted back
     * @return {string|*}
     */
    __Mcop.prototype.convertBackToOriginalUrl = function(modifiedUrl) {
        const thisProxy = this;
        if (! thisProxy.isAbsoluteUrl(modifiedUrl)) return modifiedUrl;
        const urlObjt = new URL(modifiedUrl);
        let realHost = this.getRealHost();
        let paramsPart = '';
        urlObjt.searchParams.forEach(function (value, name) {
            if (name === thisProxy.utils.originalHostName) {
                realHost = value;
                return;
            }

            if (paramsPart.length === 0) {
                paramsPart = `?${name}=${value}`;
            } else {
                paramsPart += `&${name}=${value}`;
            }
        });

        let finalResult = urlObjt.protocol + '//' + realHost;
        if (urlObjt.port.length > 0) finalResult += ':' + urlObjt.port;
        finalResult += urlObjt.pathname + paramsPart;
    ...
    __Mcop.prototype.initScope = function () {
        ...
        if (! this.scope[preparePostMsgData]) {
            this.scope[preparePostMsgData] = function(originalMsg) {
                let modifiedMsg = originalMsg;
                if (typeof originalMsg === 'object') {
                    modifiedMsg = JSON.stringify(originalMsg);
                }
                //console.log(modifiedMsg)

                if (typeof modifiedMsg === 'string') {
                    modifiedMsg = modifiedMsg.replace(/__mcopLocation/mg, 'location').replace(/__mcopOriginalFetch/mg, 'fetch');
                    modifiedMsg = modifiedMsg.replace(/__mcopOriginalOpen/mg, 'open').replace(/__mcopOriginalSharedWorker/mg, 'SharedWorker');
                    modifiedMsg = modifiedMsg.replace(/__mcopOriginalWebSocket/mg, 'WebSocket').replace(/__mcopOriginalWorker/mg, 'Worker');
                    modifiedMsg = modifiedMsg.replace(/__mcopUtils\s*,/mg, '').replace(/_mcopPreparePostMessageMsg\s*,/mg, '');
                    modifiedMsg = modifiedMsg.replace(/_mcopPreparePostMessageOrigin\s*,/mg, '');

                    if (typeof originalMsg === 'object') {
                        modifiedMsg = JSON.parse(modifiedMsg);
                    }
                }
    ...
    __Mcop.prototype.initWindow = function () {
                    ...
                    'a': {
                        modify: function () {
                            ...
                            if (typeof value === 'string') {
                                if (thisProxy.isRelativeUrl(value)) {
                                    thisElement.setAttr(name, value);
                                } else {
        ...
        this.modifyProperty(this.scope.HTMLMediaElement.prototype, 'currentSrc', function (descriptor) {
            return thisProxy.convertBackToOriginalUrl(thisProxy.modifyUrl(descriptor.get.call(this)));
        });
        ...
        this.modifyProperty(this.scope.HTMLAnchorElement.prototype, 'host', function (descriptor) {
           const host = descriptor.get.call(this);
           const href = this['href'];
           if (! thisProxy.isAbsoluteUrl(href)) return host;
           const urlObjt = new URL(href);
           return urlObjt.host;
        });

        this.modifyProperty(this.scope.HTMLAnchorElement.prototype, 'hostname', function (descriptor) {
            const hostname = descriptor.get.call(this);
            const href = this['href'];
            if (! thisProxy.isAbsoluteUrl(href)) return hostname;
            const urlObjt = new URL(href);
            return urlObjt.hostname;
        });

        this.modifyProperty(this.scope.HTMLAnchorElement.prototype, 'href', function (descriptor) {
            const href = descriptor.get.call(this);
            if (! thisProxy.isAbsoluteUrl(href)) return href;
            return thisProxy.convertBackToOriginalUrl(thisProxy.modifyUrl(href));
        });

        this.modifyProperty(this.scope.HTMLAnchorElement.prototype, 'origin', function (descriptor) {
            const hostname = descriptor.get.call(this);
            const href = this['href'];
            if (! thisProxy.isAbsoluteUrl(href)) return hostname;
            const urlObjt = new URL(href);
            return urlObjt.origin;
        });
        ...
        this.modifyProperty(this.scope.HTMLScriptElement.prototype, 'src', function (descriptor) {
            const src = descriptor.get.call(this);
            //console.log(thisProxy.convertBackToOriginalUrl(src))
            return thisProxy.convertBackToOriginalUrl(src);
        }, function (descriptor, value) {
            descriptor.set.call(this, thisProxy.modifyUrl(value));
        });
        ...
        this.modifyMethodOrConstructor(this.scope.Document.prototype, 'querySelector', function () {
            let selector = arguments[0];
            const eltFound = this[thisProxy.utils.originalPrefix + thisProxy.utils.ucFirst('querySelector')].call(this, selector);

            if (eltFound && eltFound.contentWindow) {
                new __Mcop(eltFound.contentWindow).init();
            }
        ...
        this.modifyMethodOrConstructor(this.scope.Element.prototype, 'querySelector', function () {
            let selector = arguments[0];
            const eltFound = this[thisProxy.utils.originalPrefix + thisProxy.utils.ucFirst('querySelector')].call(this, selector);
        ...
        this.modifyMethodOrConstructor(this.scope.Element.prototype, 'setAttribute', function () {
            ...
            if (/^(src|href)$/.test(attrName + '')) {
                if (/^href$/.test(attrName + '') && thisProxy.isAbsoluteUrl(attrValue)) {
        ...
        this.modifyMethodOrConstructor(this.scope.Element.prototype, 'getAttribute', function () {
            const attrName = arguments[0];
            let attrValue = this[thisProxy.utils.originalPrefix + thisProxy.utils.ucFirst('getAttribute')].call(this, attrName);
            if (/^(src)$/.test(attrName + '')) {
                if (this instanceof thisProxy.scope.HTMLScriptElement) {
                    attrValue = thisProxy.convertBackToOriginalUrl(attrValue);
                }
            }
        ...
        this.modifyProperty(this.scope.Document.prototype, 'cookie', function (descriptor) {
            ...
            if (/;/mg.test(currentCookies)) {
                ...
                for (let i = 0; i < cookiesArray.length; i++) {
                    ...
                    const expires = mcopCookie.getAttribute('expires');
