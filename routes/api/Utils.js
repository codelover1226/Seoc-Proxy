...
const internalHelpers = {
    ...
    compressFile: function (filePath, compressionMethod) {
        return new Promise(function (resolve, reject) {
            if (! (compressionMethod === 'gzip' || compressionMethod === 'deflate' || compressionMethod === 'brotli')) {
                reject(new Error('Invalid compression method. Only gzip, deflate and brotli are accepted.'));
                return;
            }
                    ...
                    try {
                        ...
                        while ((chunk = fileStream.read()) !== null) {
                            if (bufferedData) {
                                ...
                                bufferedData = Buffer.concat([chunk]);
                            }
                        }
                    } catch (e) {
                        reject(e);
                ...
                fileStream.on('end', function () {
                    try {
                        if (compressionMethod === 'gzip') {
                            resolve(zlib.gzipSync(bufferedData));
                        } else if (compressionMethod === 'deflate') {
                            resolve(zlib.deflateSync(bufferedData));
                        } else {
                            resolve(zlib.brotliCompressSync(bufferedData));
                        }
                    } catch (e) {
                        reject(e);
                    }
...
Utils.prototype.gzipCompress = async function (filePath, options = {}) {
    return await internalHelpers.compressFile(filePath, 'gzip');
};

Utils.prototype.deflateCompress = async function (filePath, options = {}) {
    return await internalHelpers.compressFile(filePath, 'deflate');
};

Utils.prototype.brotliCompress = async function (filePath, options = {}) {
    return await internalHelpers.compressFile(filePath, 'brotli');
};
...
Utils.prototype.isStaticRes = function (url) {
    const regExp = /\.(css|png|jpg|map|ico|svg|mp3|mp4|jfproj|etx|pfa|fnt|vlw|woff|fot|ttf|sfd|pfb|vfb|otf|gxf|odttf|woff2|pf2|bf|ttc|chr|bdf|fon)/i;

    return regExp.test(url + '');
};

Utils.prototype.isJsFile = function (url) {
    return /\.js\W*/i.test(url + '') && ! /\.js\w+/i.test(url + '');
};

/**
 * Returns an iterable array-like object in which each item is an object (with a single property named **index** representing
 * an index of an occurrence of **subject** within **str**). Otherwise it returns an empty object.
 * @param subject {string} the string that is searched
 * @param str {string} the string in which occurrences are searched for.
 * @return {*[]}
 */
Utils.prototype.findAllOccurrences = function (subject, str) {
    const occurrences = [];
