...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://www.seolyze.com/Login/', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
            ...
            //await page.click('a[data-target="#login"]');

            await page.focus('#username').then(async function () {
...
/**
 * Login to seolyze and save cookies in **sites/seolyze/session/cookies.json**. Returns a Promise which resolves to a hash
 * containing the following properties (site, userAgent, cookies, creationDate); other the Promise rejects with a string.
 * The login is done by submitting login credentials to https://www.seolyze.com/php_bin/security/login.php
 * @method connect
 * @return {Promise<any>}
 */
LoginAgent.prototype.connect_http = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        let postData = "username=" + username + "&password=" + password + "&stay_logged=1";
        ...
        const headers = {
            ...
            'Referer': 'https://www.seolyze.com.com/en',
            'X-Requested-With': 'XMLHttpRequest',
        };

        errorFound = false;
        const loginPath = '/php_bin/security/login.php';

        await webClient.sendRequest('post', 'www.seolyze.com', loginPath, headers, postData).then(async function (serverResponse) {
            try {
                ...
                if (serverResponse.statusCode === 200) {
                    ...
                    await utils.writeToLog("Request to https://www.seolyze.com/php_bin/security/login.php failed with status code " + serverResponse.statusCode);
                    await utils.writeToLog("Body: " + body);
                    reject("Request failed with code " + serverResponse.statusCode);
                }
            } catch (error) {
                await utils.writeToLog(error);
                reject('An error occurred whilst reading response. Retry if it persists contact admin.');
            }
        }).catch(async function (error) {
            await utils.writeToLog(error);
            reject('An error occurred whilst authenticating. Retry if it persists contact admin.');
        });
    });
};
...
LoginAgent.prototype.saveSessionCookie = async function(cookies) {
    try {
        ...
        await AppCookiesListModel.deleteOne({name: servicesDetails.seolyze.name});

        this.cookiesManager.merge(cookiesArray, this.host);
        await AppCookiesListModel.create({
            name: servicesDetails.seolyze.name,
