...
const SERVICE_MAIN_DOMAIN = 'www.seolyze.com';
const SERVICE_ROOT_DOMAIN = 'seolyze.com';
...
module.exports = async function (request, reply) {
    ...
    try {
        if (/^\/do-auto-login$/.test(request.url)) {
            await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.seolyzeUsername, request.seocromom.globalParams.seolyzePassword);
            return true;
            ...
            if (serviceDomainRegExp.test(targetedHost)) {
                if (typeof request.seocromom !== 'object' ||
                    ...
                    if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                        reply.header('location', servicesDetails.seolyze.homeUrl);
        ...
        if (! utils.isStaticRes(request.url)) {
            appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.seolyze.name}).exec();
            if (appCookiesModel)
        ...
        return reply.status(500).view('error.pug', {title: "Internal error", msg: 'Internal error'});
    }
};
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string" || url === "/" || /\/HowTo/i.test(url) || /\/login/i.test(url) || /\/UserCenter/i.test(url);
};
...
internals.removeUselessParts = function($, req) {
    ...
    const urlRegExp = /https:\/\/www\.seolyze\.com/;
...
internals.notConnected = function($) {
    let result = false;
    $('a').each(function () {
        if (/login/i.test($(this).attr('href')) || /#login/.test($(this).attr('data-target')))
            result = true;
    });
    return result;
};
