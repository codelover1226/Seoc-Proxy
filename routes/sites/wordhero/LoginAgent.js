...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://app.wordhero.co/login', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
            ...
            await page.focus('input[type="email"]').then(async function () {
                await page.keyboard.type(thisAgent.username, {delay: randDelay});
            });

            await page.focus('input[type="password"]').then(async function () {
                await page.keyboard.type(thisAgent.password, {delay: randDelay});
            });

            await page.click('input[type="checkbox"]').catch(async function (error) {
                await utils.writeToLog(error);
            });

            await page.click('button.clickable-element').catch(async function (error) {
                await utils.writeToLog(error);
            });
