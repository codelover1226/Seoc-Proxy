...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://tool.haloscan.com/sign-in', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
            ...
            await page.focus('input[name="username"]').then(async function () {
            ...
            await Promise.all([
                ...
                if (/login/.test(page.url())) {
                    ...
                    const rawCookies = await page.cookies('https://api.haloscan.com/auth/login', 'https://tool.haloscan.com/sign-in');
...
LoginAgent.prototype.saveSessionCookie_FromHeader = async function(cookies) {
    try {
        ...
        await AppCookiesListModel.deleteOne({name: servicesDetails.haloscan.name});
