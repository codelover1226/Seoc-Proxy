...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://www.paraphraser.io', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
            ...
            await page.focus('#loginemail').then(async function () {
                await page.keyboard.type(thisAgent.username, {delay: randDelay});
            });

            await page.focus('#loginpassword').then(async function () {
            ...
            const loginBackground = await page.$('#background-login');
...
LoginAgent.prototype.connect_http = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        if (thisAgent.isInLockMode()) {
            reject('A connection is already underway. Please retry later');
            return;
        }
        ...
        await webClient.sendRequest('post', 'www.paraphraser.io', loginPath, headers, postData).then(function (serverResponse) {
            try {
                let body = Buffer.concat(serverResponse.body).toString();

                if (/text\/html/.test(serverResponse.headers['content-type'] + '')) {
                    ...
                    if (serverResponse.statusCode === 200) {
                        ...
                        } else if (jsonBody.message === 'error') {
                            errorFound = true;
                            errorMsg = 'Invalid logins';
                        } else {
                            errorFound = true;
                            errorMsg = 'An unpredictable error occurred.';
                        }
                    } else {
                        errorFound = true;
...
/**
 * Gets the pre-login cookies by visiting the https://www.paraphraser.io/ endpoint.
...
LoginAgent.prototype.getPreloginDetails = function () {
    ...
    let errorFound = false;
    return new Promise(async function (resolve, reject) {
        const serverRes = await webClient.sendRequest('get', 'www.paraphraser.io', '/').catch(async function (error) {
            errorFound = true;
        ...
        if (serverRes.statusCode === 200) {
            ...
            utils.writeToLog("request to https://www.paraphraser.io/ failed with status code : " + serverRes.statusCode);
