...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://www.closerscopy.com/', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
...
/**
 * Login to closerscopy.com and save cookies in **sites/closerscopy/session/cookies.json**. Returns a Promise which resolves to a hash
...
LoginAgent.prototype.connect_http = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        const postData = `email=${username}&password=${password}&_token=${preLoginDetails.token}`;
        const headers = {
            ...
            'referer' : 'https://www.closerscopy.com/signin',
            'X-Requested-With' : 'XMLHttpRequest',
        };

        await webClient.sendRequest('post', 'www.closerscopy.com', loginPath, headers, postData).then(async function (serverResponse) {
            try {
                //console.log(serverResponse);
                const respBody = Buffer.concat(serverResponse.body).toString();
                if (serverResponse.statusCode === 302) {
                    if (/signin/.test(serverResponse.headers['location'])) {
                        reject('Login failed with invalid logins');
                    } else if (/dashboard/.test(serverResponse.headers['location'])) {
                        ...
                        utils.writeToLog(`Login to failed with status code ${serverResponse.statusCode} and body ${respBody}`);
                        reject('Login failed with inconsistent data sent by the remote server');
                    }
                } else {
                    utils.writeToLog(`Login to failed with status code ${serverResponse.statusCode} and body ${respBody}`);
                ...
                reject('An unpredictable error occurred.');
            }
        }).catch(function (error) {
            utils.writeToLog(error);
...
/**
 * Gets the pre-login cookies by visiting the https://closerscopy.com/account/login endpoint. In case of success it returns
...
LoginAgent.prototype.getPreloginCookies = function () {
    ...
    return new Promise(async function (resolve, reject) {
        try {
            ...
            await webClient.sendRequest('get', 'www.closerscopy.com', path, headers).then(function (serverResponse) {
...
LoginAgent.prototype.saveSessionCookie = async function(cookies) {
    try {
        ...
        await AppCookiesListModel.deleteOne({name: servicesDetails.closerscopy.name});
