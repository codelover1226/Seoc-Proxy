...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://onehourindexing.co/account/login', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
...
/**
 * Login to onehourindexing.co and save cookies in **sites/onehourindexing/session/cookies.json**. Returns a Promise which resolves to a hash
...
LoginAgent.prototype.connect_http = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        const loginPath = '/account/loginvalidate';
        const postData = 'emailaddress=' + username + '&password=' + password + '&roleid=+&rememberMe=true';
        const headers = {
            ...
            'referer' : 'https://onehourindexing.co/account/login',
            'X-Requested-With' : 'XMLHttpRequest',
        };

        await webClient.sendRequest('post', 'onehourindexing.co', loginPath, headers, postData).then(function (serverResponse) {
            //console.log(serverResponse);
            const respBody = Buffer.concat(serverResponse.body).toString();
            utils.writeToLog(respBody);
            if (serverResponse.statusCode === 200) {
                if (/\/account/i.test(respBody + "")) {
...
/**
 * Gets the pre-login cookies by visiting the https://onehourindexing.co/account/login endpoint. In case of success it returns
...
LoginAgent.prototype.getPreloginCookies = function () {
    ...
    return new Promise(async function (resolve, reject) {
        try {
            ...
            await webClient.sendRequest('get', 'onehourindexing.co', path, headers).then(function (serverResponse) {
                if (serverResponse.statusCode === 200) {
                    const finalResult = {
                        userAgent: userAgent,
...
LoginAgent.prototype.saveSessionCookie = async function(cookies) {
    try {
        ...
        await AppCookiesListModel.deleteOne({name: servicesDetails.onehourindexing.name});
...
LoginAgent.prototype.saveSessionCookie_FromHeader = async function(cookies) {
    try {
        ...
        this.cookiesManager.merge(cookies);
        await AppCookiesListModel.create({
            name: servicesDetails.onehourindexing.name,
            cookies: this.cookiesManager.getAllAsObject(),
        });
