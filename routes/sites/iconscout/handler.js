...
module.exports = async function (request, reply) {
    ...
    if (/^\/do-auto-login$/.test(request.url)) {
        await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.iconscoutUsername, request.seocromom.globalParams.iconscoutPassword);
        return true;
        ...
        if (serviceDomainRegExp.test(targetedHost)) {
            if (typeof request.seocromom !== 'object' ||
                ...
                if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                    reply.header('location', servicesDetails.iconscout.homeUrl);
    ...
    if (! utils.isStaticRes(request.url)) {
        appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.iconscout.name}).exec();
        if (appCookiesModel)
    ...
    if (typeof serverRes.headers['set-cookie'] !== "undefined" && ! utils.isStaticRes(request.url)) {
        ...
        await AppCookiesListModel.findOneAndUpdate(
            {name: servicesDetails.iconscout.name},
    ...
    if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
        ...
        if (body.length > 0) {
            if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                await AppCookiesListModel.deleteOne({name: servicesDetails.iconscout.name});
                reply.header('location', '/');
                return reply.code(statusCode).send("Redirecting...");
            }
...
internals.removeUselessParts = function($, req) {
    ...
    const userOptsElt = $('#ddUserOptions');
...
internals.injectJsAdaptor = async function($, req) {
    const regExp = new RegExp("sites" + "\\" + path.sep + "iconscout");
    let fullPath = __dirname.replace(regExp, "api/frontend-compos/iconscout-adapter.js");
    ...
    if (req.seocromom.currentUser.role !== 'admin') {
        ...
        headerBlock.append('<style type="text/css" class="seoc-incss">#ddUserOptions{display: none !important;}</style>');
    }
