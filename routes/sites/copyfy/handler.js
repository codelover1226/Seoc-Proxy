...
const SERVICE_MAIN_DOMAIN = 'www.copyfy.io';
const SERVICE_ROOT_DOMAIN = 'copyfy.io';
...
module.exports = async function (request, reply) {
    try {
        ...
        if (serviceDomainRegExp.test(targetedHost)) {
            if (typeof request.seocromom !== 'object' ||
                ...
                if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                    reply.header('location', servicesDetails.copyfy.homeUrl);
        ...
        let appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.copyfy.name}).exec();
        if (appCookiesModel)
        ...
        if (typeof serverRes.headers['set-cookie'] !== "undefined" && ! utils.isStaticRes(request.url)) {
            ...
            await AppCookiesListModel.findOneAndUpdate(
                {name: servicesDetails.copyfy.name},
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && handlerHelpers.isHtml(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteMany({name: servicesDetails.copyfy.name});
                    reply.header('location', servicesDetails.copyfy.homeUrl);
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string"
        || /\/login/.test(url) || /\/reset\/password/.test(url)
        || /\/checkout\/register/.test(url) || /\/dashboard\/settings/.test(url) ||
        /\/dashboard\/plans/.test(url) || /\/logout/.test(url);
};

internals.removeUselessParts = function($, req) {
    if (req.seocromom.currentUser.role !== 'admin') {
        $('.dropdown-user-link').remove();
    }
    return $.html();
};