...
const webClient = require("../../api/WebClient").create(false);
...
const SERVICE_MAIN_DOMAIN = 'espinner.net';
const SERVICE_ROOT_DOMAIN = 'espinner.net';
...
module.exports = async function (request, reply) {
    ...
    try {
        if (/^\/do-auto-login$/.test(request.url)) {
            await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.espinnerUsername, request.seocromom.globalParams.espinnerPassword);
            return true;
            ...
            if (serviceDomainRegExp.test(targetedHost)) {
                if (typeof request.seocromom !== 'object' ||
                    ...
                    if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                        reply.header('location', servicesDetails.espinner.homeUrl);
        ...
        if (! utils.isStaticRes(request.url)) {
            appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.espinner.name}).exec();
        ...
        if (typeof serverRes.headers['set-cookie'] !== "undefined" && ! utils.isStaticRes(request.url)) {
            ...
            if (appCookiesModel) {
                await AppCookiesListModel.updateOne({_id: appCookiesModel._id},
                    {
                        name: servicesDetails.espinner.name,
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && internals.contentIsHTML(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteOne({name: servicesDetails.espinner.name});
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string" || url === '/' ||
        /\/miembros\/profile/.test(url) || /\/miembros\/login\/logout/.test(url) ||
        /\/miembros\/signup/.test(url);
};
...
internals.removeUselessParts = function($, req) {
    ...
    const urlRegExp = /http:\/\/www\.espinner\.com/;

    $('.control-panel__account-bar').remove();
    ...
    /*links.each(function () {
        ...
        if (/^\//.test(href)) {
            $(this).attr('href', `http://${SERVICE_MAIN_DOMAIN}${href}`);
            $(this).attr("__mcopp", '1');
        }
    ...
    scripts.each(function () {
        ...
        if (/^\//.test(src)) {
            $(this).attr('src', `http://${SERVICE_MAIN_DOMAIN}${src}`);
            $(this).attr("__mcopp", '1');
        }
    });
