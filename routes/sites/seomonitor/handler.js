...
const SERVICE_MAIN_DOMAIN = 'app.seomonitor.com';
const SERVICE_ROOT_DOMAIN = 'seomonitor.com';
    ...
    try {
        ...
        if (handlerHelpers.urlContainsOriginalHost(request.url)) {
            ...
            /*if ((request.method + '').toLowerCase() === 'get') {
                let redirectUrl = targetedUrl + ((targetedUrl.includes('&')) ? '&' : '?');
                redirectUrl += 'original-host=' + SERVICE_MAIN_DOMAIN;
                reply.code(302).header('location', redirectUrl);
                return;
            }*/
            ...
            if (serviceDomainRegExp.test(targetedHost)) {
                if (typeof request.seocromom !== 'object' ||
                    ...
                    if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                        reply.header('location', servicesDetails.seomonitor.homeUrl);
        ...
        if (! utils.isStaticRes(request.url)) {
            appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.seomonitor.name}).exec();
            if (appCookiesModel) {
            ...
            /*if (request.seocromom['currentUser'].role === "admin") {
                ...
                await AppCookiesListModel.findOneAndUpdate(
                    {name: servicesDetails.seomonitor.name},
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteMany({name: servicesDetails.seomonitor.name});
                    reply.header('location', servicesDetails.seomonitor.homeUrl);
            ...
            body = body.replace(/www\.seomonitor\.com/mg, request.seocromom.currentDomain);
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string" ||
        /\/account/i.test(url) || /\/my_account/i.test(url) ||
        /\/user/i.test(url) ||
        /\/profile/i.test(url) || /\/oauth-logout/i.test(url);
};

internals.injectJsAdaptor = async function($, req) {
    const regExp = new RegExp("sites" + "\\" + path.sep + "seomonitor");
    let fullPath = __dirname.replace(regExp, "api/frontend-compos/seomonitor-adapter.js");
    ...
    if (req.seocromom.currentUser.role !== 'admin') {
        ...
        headerBlock.append('<style type="text/css" class="seoc-incss">.user-menu-holder{display: none !important;}</style>');
    }
