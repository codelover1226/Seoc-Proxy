...
const SERVICE_MAIN_DOMAIN = 'www.spinrewriter.com';
const SERVICE_ROOT_DOMAIN = 'spinrewriter.com';
...
module.exports = async function (request, reply) {
    ...
    try {
        if (/^\/do-auto-login$/.test(request.url)) {
            await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.spinrewriterUsername, request.seocromom.globalParams.spinrewriterPassword);
            return true;
            ...
            if (serviceDomainRegExp.test(targetedHost)) {
                if (typeof request.seocromom !== 'object' ||
                    ...
                    if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                        reply.header('location', servicesDetails.spinrewriter.homeUrl);
        ...
        if (! utils.isStaticRes(request.url)) {
            appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.spinrewriter.name}).exec();
            if (appCookiesModel)
        ...
        if (typeof serverRes.headers['set-cookie'] !== "undefined" && ! utils.isStaticRes(request.url)) {
            ...
            if (appCookiesModel) {
                await AppCookiesListModel.updateOne({_id: appCookiesModel._id},
                    {
                        name: servicesDetails.spinrewriter.name,
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && internals.contentIsHTML(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteOne({name: servicesDetails.spinrewriter.name});
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string" || url === '/' ||
        /\/log-out/.test(url) || /\/cp-account/.test(url) || /\/cp-account-settings/.test(url) ||
        /\/cp-affiliate-program/.test(url) || /\/cp-api/.test(url) || /\/cp-gold-membership/.test(url) ||
        /\/cp-wordpress-license/.test(url);
};
...
internals.removeUselessParts = function($, req) {
    ...
    const urlRegExp = /https:\/\/www\.spinrewriter\.com/;
