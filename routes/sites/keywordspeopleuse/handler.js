...
const SERVICE_MAIN_DOMAIN = 'keywordspeopleuse.com';
const SERVICE_ROOT_DOMAIN = 'keywordspeopleuse.com';
...
module.exports = async function (request, reply) {
    try {
        ...
        let appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.keywordspeopleuse.name}).exec();
        if (appCookiesModel)
        ...
        if (typeof request.seocromom !== 'object' ||
            ...
            if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                reply.header('location', servicesDetails.keywordspeopleuse.homeUrl);
        ...
        if (typeof serverRes.headers['set-cookie'] !== "undefined" && ! utils.isStaticRes(request.url)) {
            ...
            await AppCookiesListModel.findOneAndUpdate(
                {name: servicesDetails.keywordspeopleuse.name},
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && handlerHelpers.isHtml(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteMany({name: servicesDetails.keywordspeopleuse.name});
                    reply.header('location', servicesDetails.keywordspeopleuse.homeUrl);
                    return reply.code(302).send("Redirecting...");
                }
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string"
        || /\/signup/.test(url) || /\/login/.test(url)
        || /\/logout/.test(url) || /\/pricing/.test(url)
        || /\/team/.test(url) || /\/settings/.test(url);
};
...
internals.injectJsAdaptor = async function($, req) {
    ...
    const regExp = new RegExp("sites" + "\\" + path.sep + "keywordspeopleuse");
    let fullPath = __dirname.replace(regExp, "api/frontend-compos/keywordspeopleuse-adapter.js");
    ...
    headerBlock.append('<style type="text/css" class="seoc-incss">.Header_avatar-container__4svZT{display: none !important;}</style>');
    return $.html();
};