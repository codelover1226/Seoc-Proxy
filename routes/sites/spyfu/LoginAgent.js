...
/**
 * This class is responsible of logging into https://www.spyfu.com/ using a username and password.
 * @class LoginAgent
 * @constructor
 */
function LoginAgent() {
    this.cookiesManager = cookiesManagerCreator.create({});
    this.host = "www.spyfu.com";
}
...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://www.spyfu.com/auth/login', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
...
LoginAgent.prototype.saveSessionCookie = async function(cookies) {
    try {
        ...
        await AppCookiesListModel.deleteOne({name: servicesDetails.spyfu.name});
