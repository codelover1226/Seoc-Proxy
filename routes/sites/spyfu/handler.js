...
const SERVICE_MAIN_DOMAIN = 'www.spyfu.com';
const SERVICE_ROOT_DOMAIN = 'spyfu.com';
...
module.exports = async function (request, reply) {
    ...
    if (/^\/do-auto-login$/.test(request.url)) {
        await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.spyfuUsername, request.seocromom.globalParams.spyfuPassword);
        return true;
    ...
    if (! utils.isStaticRes(request.url)) {
        appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.spyfu.name}).exec();
        if (appCookiesModel)
    ...
    if (typeof serverRes.headers['set-cookie'] !== "undefined" && ! utils.isStaticRes(request.url)) {
        ...
        if (appCookiesModel) {
            await AppCookiesListModel.updateOne({_id: appCookiesModel._id},
                {
                    name: servicesDetails.spyfu.name,
