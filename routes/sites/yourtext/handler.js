...
const SERVICE_MAIN_DOMAIN = 'yourtext.guru';
const SERVICE_ROOT_DOMAIN = 'yourtext.guru';
...
module.exports = async function (request, reply) {
    try {
        if (/^\/do-auto-login$/.test(request.url)) {
            await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.yourtextUsername, request.seocromom.globalParams.yourtextPassword);
            return true;
        ...
        //we get current user only for non static resources requested on www.yourtext.com
            ...
            if (serviceDomainRegExp.test(targetedHost)) {
                ...
                if (typeof request.seocromom !== 'object' ||
                    ...
                    if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                        reply.header('location', servicesDetails.yourtext.homeUrl);
        ...
        if (! utils.isStaticRes(request.url)) {
            appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.yourtext.name}).exec();
            if (appCookiesModel)
        ...
        if (/^\/out$/.test(request.url + "") && request.seocromom['currentUser'].role === "admin") {
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string" || /\/profil|\/login|\/out|\/register|\/tarifs|\/support|\/privacy-policy/i.test(url);
};
