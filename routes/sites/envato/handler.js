...
const SERVICE_MAIN_DOMAIN = 'elements.envato.com';
const SERVICE_ROOT_DOMAIN = 'envato.com';
...
module.exports = async function (request, reply) {
    try {
        ...
        //we get current user only for non static resources requested on www.envato.com
            ...
            if (serviceDomainRegExp.test(targetedHost)) {
                ...
                if (typeof request.seocromom !== 'object' ||
                    ...
                    if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                        reply.header('location', servicesDetails.envato.homeUrl);
        ...
        if (! utils.isStaticRes(request.url)) {
            appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.envato.name}).exec();
            if (appCookiesModel)
                cookiesManager.setOldCookies(appCookiesModel.cookies);

            //const clientSideCookies = handlerHelpers.getClientSideCookies(request.headers['cookie'], cookiesHost);
        ...
        if (typeof serverRes.headers['set-cookie'] !== "undefined" && ! utils.isStaticRes(request.url)) {
            ...
            if (appCookiesModel) {
                await AppCookiesListModel.updateOne({_id: appCookiesModel._id},
                    {
                        name: servicesDetails.envato.name,
        ...
        if (/^\/sign-out$/.test(request.url + "") && request.seocromom['currentUser'].role === "admin") {
            await AppCookiesListModel.deleteMany({name: servicesDetails.envato.name});
        }
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && handlerHelpers.isHtml(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteOne({name: servicesDetails.envato.name});
                    reply.header('location', servicesDetails.envato.homeUrl);
        ...
        } else if (realFullUrl.includes('download.json')) {
            const downloadObjt = JSON.parse(body);
            if (downloadObjt.data && downloadObjt.data.attributes && downloadObjt.data.attributes.downloadUrl) {
                downloadObjt.data.attributes.downloadUrl = handlerHelpers.modifyUrl(downloadObjt.data.attributes.downloadUrl,
                    request.seocromom.currentDomain);
            }

            body = JSON.stringify(downloadObjt);
        }

        /*await utils.writeToLog(realFullUrl);
        await utils.writeToLog(JSON.stringify(serverRes.headers));
        await utils.writeToLog(body + '\n');*/
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string" || /\/sign-in|\/sign-out|\/account/i.test(url);
};
