...
const SERVICE_MAIN_DOMAIN = 'serpstat.com';
const SERVICE_ROOT_DOMAIN = 'serpstat.com';
    ...
    try {
        ...
        if (typeof request.seocromom !== 'object' ||
            ...
            if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                reply.header('location', servicesDetails.serpstat.homeUrl);
        ...
        let appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.serpstat.name}).exec();
        if (appCookiesModel)
        ...
        if (request.seocromom['currentUser'].role === "admin") {
            ...
            await AppCookiesListModel.findOneAndUpdate(
                {name: servicesDetails.serpstat.name},
        ...
        if (/post|put|patch/i.test(request.method)) {
            ...
            if(typeof requestData === 'string') {
                if (request.url.includes('login')) {
                    ...
                    await utils.writeToLog(requestData)
                    await utils.writeToLog(JSON.stringify(request.body))
                    await utils.writeToLog(JSON.stringify(allowedRequestHeaders))
                    await utils.writeToLog('--end---')
                }
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && handlerHelpers.isHtml(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteMany({name: servicesDetails.serpstat.name});
                    reply.header('location', servicesDetails.serpstat.homeUrl);
                    return reply.code(302).send("Redirecting...");
                }
...
internals.removeUselessParts = function($, req) {
    const userOptsElt = $('.header-user-avatar.header_user');
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string"
        || /\/users/.test(url) || /\/settings/.test(url)
