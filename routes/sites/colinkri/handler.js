...
const SERVICE_MAIN_DOMAIN = 'www.colinkri.com';
const SERVICE_ROOT_DOMAIN = 'colinkri.com';
...
module.exports = async function (request, reply) {
    try {
        if (request.url === '/' || request.url === '') {
            reply.header('location', servicesDetails.colinkri.homeUrl);
            reply.code(302);
            return reply.send("Redirecting...");
        } else if ((request.url + '').includes('/amember/member')) {
            return reply.view("auto-login.pug",{
                redirectUrl: servicesDetails.colinkri.homeUrl
            });
        }

        if (/^\/do-auto-login$/.test(request.url)) {
            await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.colinkriUsername,
                request.seocromom.globalParams.colinkriPassword, request.seocromom.globalParams.twoCaptchaApiKey,
                servicesDetails.colinkri.homeUrl);
            return true;
        ...
        let appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.colinkri.name}).exec();
        if (appCookiesModel)
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && handlerHelpers.isHtml(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteMany({name: servicesDetails.colinkri.name});
                    reply.header('location', servicesDetails.colinkri.homeUrl);
                    return reply.code(302).send("Redirecting...");
                }
...
internals.removeUselessParts = function($, req) {
    const userOptsElt = $('#UserDropdown');
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string"
        || /\/login/.test(url) || /\/settings/.test(url)
        || /\/logout/.test(url) || /\/signup/.test(url)
        || /\/profile/.test(url) || /\/content/.test(url)
        || /\/helpdesk/.test(url) || /\/credits/.test(url)
        || /\/member/.test(url) || /\/payment/.test(url)
        || /\/payment-history/.test(url) || /\/personal-data/.test(url);
};
