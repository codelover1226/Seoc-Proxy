...
const SERVICE_MAIN_DOMAIN = 'bigspy.com';
const SERVICE_ROOT_DOMAIN = 'bigspy.com';
...
module.exports = async function (request, reply) {
    ...
    try {
        ...
        if (/^\/do-auto-login$/.test(request.url)) {
            await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.bigspyUsername, request.seocromom.globalParams.bigspyPassword);
            return true;
            ...
            if (serviceDomainRegExp.test(targetedHost)) {
                if (typeof request.seocromom !== 'object' ||
                    ...
                    if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                        reply.header('location', servicesDetails.bigspy.homeUrl);
        ...
        if (! utils.isStaticRes(request.url)) {
            appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.bigspy.name}).exec();
            if (appCookiesModel)
        ...
        if (handlerHelpers.mimeIsJs(serverRes.headers['content-type'] + "") ||
            ...
            if (body.length > 0 && handlerHelpers.isHtml(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteMany({name: servicesDetails.bigspy.name});
                    reply.header('location', servicesDetails.bigspy.homeUrl);
                    return reply.code(302).send("Redirecting...");
                }
...
internals.injectJsAdaptor = async function($) {
    const regExp = new RegExp("sites" + "\\" + path.sep + "bigspy");
    let fullPath = __dirname.replace(regExp, "api/frontend-compos/bigspy-adapter.js");
