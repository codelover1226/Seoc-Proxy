...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://bigspy.com/user/login', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
            ...
            await page.focus('#loginform-username').then(async function () {
...
/**
 * Gets the pre-login cookies by visiting the https://bigspy.com/user/login endpoint.
...
LoginAgent.prototype.getPreloginDetails = function () {
    ...
    return new Promise(async function (resolve, reject) {
        const serverRes = await webClient.sendRequest('get', 'bigspy.com', '/user/login').catch(async function (error) {
            errorFound = true;
        ...
        if (serverRes.statusCode === 200) {
            ...
            utils.writeToLog("request to https://bigspy.com/user/login failed with status code : " + serverRes.statusCode);
...
LoginAgent.prototype.saveSessionCookie = async function(cookies) {
    try {
        ...
        await AppCookiesListModel.deleteOne({name: servicesDetails.bigspy.name});
        this.cookiesManager.merge(cookiesArray, this.host);
        await AppCookiesListModel.create({
            name: servicesDetails.bigspy.name,
