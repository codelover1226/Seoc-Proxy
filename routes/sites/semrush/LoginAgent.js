...
LoginAgent.prototype.connect = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            await page.goto('https://www.semrush.com/login/?src=header&redirect_to=%2F', {waitUntil: 'load', timeout : defaultTimeout}).catch(async function (error) {
                utils.writeToLog(error);
                await browser.close(true).catch(function (error) {
                    utils.writeToLog(error);
                });
            ...
            if ($(":contains('Checking your browser')").length > 0) {
...
LoginAgent.prototype.connect_bypassRecaptcha = function (username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        if (thisAgent.isInLockMode()) {
            reject('A connection to semrush is already underway. Please retry later');
            return;
        }
        ...
        if (errorFound) {
            reject("Failed to get pre login cookies");
            thisAgent.leaveLockMode();
        }
        ...
        //we get a recaptcha response for the semrush login page
        await twoCaptcaClient.solveCaptchaV2("6Ldw6DYUAAAAACFCNmvsT32P6VPVonpjbSS7XTA9", "https://www.semrush.com/login/?src=header&redirect_to=%2Fnavigator%2F", true)
        ...
        let postData = {
            ...
            "user-agent-hash": crypto.createHash('sha1').update(username).digest('hex').substr(0,32)
        };
        ...
        const headers = {
            ...
            'referer': 'https://www.semrush.com/login/?src=header&redirect_to=%2Fnavigator%2F',
        };
        ...
        await webClient.sendRequest('post', 'www.semrush.com', loginPath, headers, JSON.stringify(postData)).then(function (serverResponse) {
            try {
                ...
                if (/application\/json/.test(serverResponse.headers['content-type'] + '')) {
                    ...
                    if (serverResponse.statusCode === 201) {
                        ...
                        errorFound = true;
                        errorMsg = 'semrush server says: ' + jsonBody.message;
                    } else {
...
/**
 * Gets the pre-login cookies by visiting the https://www.semrush.com/login/?src=header&redirect_to=%2Fnavigator%2F endpoint.
...
LoginAgent.prototype.getPreloginDetails = function () {
    ...
    let errorFound = false;
    return new Promise(async function (resolve, reject) {
        const serverRes = await webClient.sendRequest('get', 'www.semrush.com', '/login/?src=header&redirect_to=%2Fnavigator%2F').catch(async function (error) {
            errorFound = true;
        ...
        if (serverRes.statusCode === 200) {
            ...
            /*resolve({
                cookies: serverRes.headers['set-cookie'],
                csrfToken: hiddenInput.val()
            });*/
            resolve(serverRes.headers['set-cookie']);
        } else {
            utils.writeToLog("request to https://www.semrush.com/login/?src=header&redirect_to=%2Fnavigator%2F failed with status code : " + serverRes.statusCode);
...
LoginAgent.prototype.saveSessionCookie = async function(cookies) {
    try {
        ...
        await AppCookiesListModel.deleteOne({name: servicesDetails.semrush.name});
