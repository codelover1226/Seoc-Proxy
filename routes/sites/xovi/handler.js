...
const SERVICE_MAIN_DOMAIN = 'suite.xovi.net';
const SERVICE_ROOT_DOMAIN = 'xovi.net';
...
module.exports = async function (request, reply) {
    try {
        ...
        if (/^\/do-auto-login$/.test(request.url)) {
            await internals.doAutoLogin(loginAgent, reply, request.seocromom.globalParams.xoviUsername,
                request.seocromom.globalParams.xoviPassword, servicesDetails.xovi.homeUrl);
            return true;
        ...
        if (serviceDomainRegExp.test(targetedHost)) {
            if (typeof request.seocromom !== 'object' ||
                ...
                if (request.seocromom.currentUser.role !== 'admin' && internals.isForbiddenUrl(request.url)) {
                    reply.header('location', servicesDetails.xovi.homeUrl);
        ...
        let appCookiesModel = await AppCookiesListModel.findOne({name: servicesDetails.xovi.name}).exec();
        if (appCookiesModel)
        ...
        if (typeof serverRes.headers['location'] !== "undefined") {
            if (/\/login/.test(serverRes.headers['location']) && ! utils.isStaticRes(request.url)) {
                return reply.view("auto-login.pug",{
                    redirectUrl: servicesDetails.xovi.homeUrl
                });
            }
        ...
        if (handlerHelpers.mimeIsHtml(serverRes.headers['content-type'] + '')) {
            ...
            if (body.length > 0 && handlerHelpers.isHtml(body)) {
                if (statusCode === 400 && /Request\sHeader\sOr\sCookie\sToo\sLarge/m.test(body)) {
                    await AppCookiesListModel.deleteMany({name: servicesDetails.xovi.name});
                    reply.header('location', servicesDetails.xovi.homeUrl);
...
internals.isForbiddenUrl = function(url) {
    return typeof url !== "string"
        || /\/user\/login/.test(url) || /\/user\/register/.test(url)
        || /\/user\/forgot-password/.test(url) || /\/user\/logout/.test(url) ||
        /\/account\//.test(url) || /\/usermanager/.test(url);
};

internals.removeUselessParts = function($, req) {
    if (req.seocromom.currentUser.role !== 'admin') {
        $('.user.dropdown-button').remove();
    }
